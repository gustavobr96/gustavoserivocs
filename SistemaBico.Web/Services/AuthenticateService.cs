using Hanssens.Net;
using Microsoft.AspNetCore.Authentication;
using Microsoft.AspNetCore.Authentication.Cookies;
using Microsoft.AspNetCore.Http;
using SistemaBico.Web.Models.Reponse;
using SistemaBico.Web.Services.Interfaces;
using System.IdentityModel.Tokens.Jwt;
using System.Net.Http.Headers;
using System.Security.Claims;

namespace SistemaBico.Web.Services
{
    public class AuthenticateService : IAuthenticateService
    {
        private readonly ILocalStorageService _localStorageService;

        public AuthenticateService(ILocalStorageService localStorageService)
        {
            _localStorageService = localStorageService;
        }

        public async Task Login(HttpContext ctx, LoginResponse user, bool? professional = false)
        {
            string _imageDefault = "";

            var claims = new List<Claim>();
            //claims.Add(new Claim("Name", user.User.Name));
            //claims.Add(new Claim("Email", user.User.Email));
            ////claims.Add(new Claim("Perfil", user.User.Email));
            //claims.Add(new Claim("Token", user.Token));
            //claims.Add(new Claim("Id", user.User.Id.ToString()));
            //claims.Add(new Claim("ClientId", user.User.ClientId.ToString()));

            var perfilPicture = user.User.PerfilPicture != null && user.User.PerfilPicture.Length > 0
              ? Convert.ToBase64String(user.User.PerfilPicture)
              : _imageDefault;

            //_localStorageService.SetLocalStorage("Name", user.User.Name);
            //_localStorageService.SetLocalStorage("Email", user.User.Email);
            //_localStorageService.SetLocalStorage("Token", user.Token);
            //_localStorageService.SetLocalStorage("Id", user.User.Id.ToString());
            //_localStorageService.SetLocalStorage("ClientId", user.User.ClientId.ToString());
            //_localStorageService.SetLocalStorage("PerfilPicture", perfilPicture);
            ctx.Session.SetString("Name", user.User.Name);
            ctx.Session.SetString("Email", user.User.Email);
            ctx.Session.SetString("Token", user.Token);
            ctx.Session.SetString("Id", user.User.Id.ToString());
            ctx.Session.SetString("ClientId", user.User.ClientId.ToString());
            ctx.Session.SetString("PerfilPicture", perfilPicture);
            ctx.Session.SetString("PerfilProfessional", professional.ToString());

            var tokenHandler = new JwtSecurityTokenHandler();
            var token = tokenHandler.ReadJwtToken(user.Token);

            var claimsIdentity =
                new ClaimsPrincipal(
                    new ClaimsIdentity(
                        claims,
                        CookieAuthenticationDefaults.AuthenticationScheme
                    )
                );

            var authProperties = new AuthenticationProperties
            {
                ExpiresUtc = DateTime.Now.AddDays(1),
                IssuedUtc = DateTime.Now
            };


            await ctx.SignInAsync(CookieAuthenticationDefaults.AuthenticationScheme, claimsIdentity, authProperties);
        }

        public async Task<HttpClient> TokenAuth(HttpContext ctx, HttpClient client)
        {
            try
            {
                var claims = ctx.Session.GetString("Token"); ;
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", claims);
                return await Task.FromResult(client);
            }
            catch
            {
                return null;
            }
        }
        public async Task<bool> VerifyAuthenticated(HttpContext ctx)
        {
            try
            {
                var token = ctx.Session.GetString("Token");
                return string.IsNullOrEmpty(token) ? await Task.FromResult(true) : await Task.FromResult(false);
            }
            catch
            {
                return true;
            }
          
        }
        public async Task Logoff(HttpContext ctx)
        {
            await ctx.SignOutAsync(CookieAuthenticationDefaults.AuthenticationScheme);
        }
        public async Task<string> ObterClientIdLogado(HttpContext contexto)
        {
            try
            {
                var id = contexto.Session.GetString("ClientId");
                return id != null ? await Task.FromResult(id) : Guid.Empty.ToString();
            }
            catch
            {
                return Guid.Empty.ToString();
            }
          
        }
    }
}

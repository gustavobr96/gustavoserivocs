...
{
    public class QueueConsumerApplyProfessional : BackgroundService
    {
        ...
        public QueueConsumerApplyProfessional(
             ...
             IServiceScopeFactory serviceScopeFactory)
        {
            ...
            _channel.QueueDeclare(
                    queue: MessageType.ApplyProfessional.GetDescription(),
                    durable: false,
                    exclusive: false,
                    autoDelete: false,
                    arguments: null
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                consumer.Received += async (sender, eventArgs) =>
                {
                    ...
                    var message = JsonConvert.DeserializeObject<ApplyProfessionalCommand>(contentString);
                    _ = await _mediator.Send(message, stoppingToken);
                };

                _channel.BasicConsume(MessageType.ApplyProfessional.GetDescription(), true, consumer);
            }
            catch (Exception e)
            {

...
{
    public class QueuePublishCancelProfessionalCommandHandler : IRequestHandler<QueueCancelApplyProfessionalCommand, Result>
    {
        private readonly Generics.Interfaces.INotification _Notification;
        private ConnectionFactory _ConnectionFactory { get; set; }
        public QueuePublishCancelProfessionalCommandHandler(Generics.Interfaces.INotification notification, IConnectFactory connectionFactory)
        {
            _Notification = notification;
            _ConnectionFactory = connectionFactory.Get();
        }

        public async Task<Result> Handle(QueueCancelApplyProfessionalCommand request, CancellationToken cancellationToken)
        {
            try
            {
                using (var connection = _ConnectionFactory.CreateConnection())
                {
                    ...
                    {
                        var queueName = MessageType.CancelApplyProfessional.GetDescription();

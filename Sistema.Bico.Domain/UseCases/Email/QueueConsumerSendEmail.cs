...
{
    public class QueueConsumerSendEmail : BackgroundService
    {
             ...
             IServiceScopeFactory serviceScopeFactory)
        {
            ...
            _channel.QueueDeclare(
                    queue: MessageType.SendEmail.GetDescription(),
                    durable: false,
                    exclusive: false,
                    autoDelete: false,
                    arguments: null
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                consumer.Received += async (sender, eventArgs) =>
                {
                    ...
                    var message = JsonConvert.DeserializeObject<SendEmailCommand>(contentString);
                    _ = await _mediator.Send(message, stoppingToken);
                };

                _channel.BasicConsume(MessageType.SendEmail.GetDescription(), true, consumer);
            }
            catch (Exception e)
            {

...
{
    public class QueuePublishRegisterClientCommandHandler : IRequestHandler<QueueAddClientCommand, Result>
    {
        private readonly Generics.Interfaces.INotification _Notification;
        private ConnectionFactory _ConnectionFactory { get; set; }

        public QueuePublishRegisterClientCommandHandler(Generics.Interfaces.INotification Notification,
        ...
        public async Task<Result> Handle(QueueAddClientCommand request, CancellationToken cancellationToken)
        {
            try
            {
                using (var connection = _ConnectionFactory.CreateConnection())
                {
                    ...
                    {
                        var queueName = MessageType.RegisterCliente.GetDescription();

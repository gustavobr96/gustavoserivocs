...
{
    public class QueueConsumerForgotClient : BackgroundService
    {
        ...
        public QueueConsumerForgotClient(
             ...
             IServiceScopeFactory serviceScopeFactory)
        {
            ...
            _channel.QueueDeclare(
                    queue: MessageType.Forgot.GetDescription(),
                    durable: false,
                    exclusive: false,
                    autoDelete: false,
                    arguments: null
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                consumer.Received += async (sender, eventArgs) =>
                {
                    ...
                    var message = JsonConvert.DeserializeObject<ForgotCommand>(contentString);
                    _ = await _mediator.Send(message, stoppingToken);
                };

                _channel.BasicConsume(MessageType.Forgot.GetDescription(), true, consumer);
            }
            catch (Exception e)
            {

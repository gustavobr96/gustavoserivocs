...
{
    public class QueuePublishRegisterWorkerCommandHandler : IRequestHandler<QueueAddWorkerCommand, Result>
    {
        private readonly Generics.Interfaces.INotification _Notification;
        private ConnectionFactory _ConnectionFactory { get; set; }

        public QueuePublishRegisterWorkerCommandHandler(Generics.Interfaces.INotification notification, IConnectFactory connectionFactory)
        {
            _Notification = notification;
            _ConnectionFactory = connectionFactory.Get();
        }

        public async Task<Result> Handle(QueueAddWorkerCommand request, CancellationToken cancellationToken)
        {
            try
            {
                using (var connection = _ConnectionFactory.CreateConnection())
                {
                    ...
                    {
                        var queueName = MessageType.RegisterWorker.GetDescription();
            ...
            {
                _Notification.Handle("Erro ao enfileirar o cadastro de cliente!");

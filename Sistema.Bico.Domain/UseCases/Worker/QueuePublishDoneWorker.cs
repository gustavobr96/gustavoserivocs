...
{
    public class QueuePublishDoneWorkerCommandHandler : IRequestHandler<QueueDoneWorkerCommand, Result>
    {
        private readonly Generics.Interfaces.INotification _Notification;
        private ConnectionFactory _ConnectionFactory { get; set; }

        public QueuePublishDoneWorkerCommandHandler(Generics.Interfaces.INotification notification, IConnectFactory connectionFactory)
        {
            _Notification = notification;
            _ConnectionFactory = connectionFactory.Get();
        }

        public async Task<Result> Handle(QueueDoneWorkerCommand request, CancellationToken cancellationToken)
        {
            try
            {
                using (var connection = _ConnectionFactory.CreateConnection())
                {
                    ...
                    {
                        var queueName = MessageType.DoneWorker.GetDescription();
            ...
            {
                _Notification.Handle("Erro ao enfileirar o done worker!");

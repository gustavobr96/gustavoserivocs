...
{
    public class QueueConsumerApplyWorker : BackgroundService
    {
        ...
        public QueueConsumerApplyWorker(
             ...
             IServiceScopeFactory serviceScopeFactory)
        {
            ...
            _channel.QueueDeclare(
                    queue: MessageType.ApplyWorker.GetDescription(),
                    durable: false,
                    exclusive: false,
                    autoDelete: false,
                    arguments: null
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                consumer.Received += async (sender, eventArgs) =>
                {
                    ...
                    var message = JsonConvert.DeserializeObject<ApplyWorkerCommand>(contentString);
                    _ = await _mediator.Send(message, stoppingToken);
                };

                _channel.BasicConsume(MessageType.ApplyWorker.GetDescription(), true, consumer);
            }
            catch (Exception e)
            {

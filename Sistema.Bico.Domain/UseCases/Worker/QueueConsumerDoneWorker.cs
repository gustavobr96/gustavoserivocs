...
namespace Sistema.Bico.Domain.UseCases.Worker
{
    ...
    public class QueueConsumerDoneWorker : BackgroundService
    {
        ...
        public QueueConsumerDoneWorker(
             ...
             IServiceScopeFactory serviceScopeFactory)
        {
            ...
            _channel.QueueDeclare(
                    queue: MessageType.DoneWorker.GetDescription(),
                    durable: false,
                    exclusive: false,
                    autoDelete: false,
                    arguments: null
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                consumer.Received += async (sender, eventArgs) =>
                {
                    ...
                    var message = JsonConvert.DeserializeObject<DoneWorkerCommand>(contentString);
                    _ = await _mediator.Send(message, stoppingToken);
                };

                _channel.BasicConsume(MessageType.DoneWorker.GetDescription(), true, consumer);
            }
            catch (Exception e)
            {

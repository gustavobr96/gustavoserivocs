...
{
    public class QueuePublishApplyWorkerCommandHandler : IRequestHandler<QueueApplyWorkerCommand, Result>
    {
        private readonly Generics.Interfaces.INotification _Notification;
        private ConnectionFactory _ConnectionFactory { get; set; }

        public QueuePublishApplyWorkerCommandHandler(Generics.Interfaces.INotification notification, IConnectFactory connectionFactory)
        {
            _Notification = notification;
            _ConnectionFactory = connectionFactory.Get();
        }

        public async Task<Result> Handle(QueueApplyWorkerCommand request, CancellationToken cancellationToken)
        {
            try
            {
                using (var connection = _ConnectionFactory.CreateConnection())
                {
                    ...
                    {
                        var queueName = MessageType.ApplyWorker.GetDescription();
            ...
            {
                _Notification.Handle("Erro ao enfileirar o apply worker!");

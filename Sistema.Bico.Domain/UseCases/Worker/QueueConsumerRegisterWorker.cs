...
{
    public class QueueConsumerRegisterWorker : BackgroundService
    {
        ...
        public QueueConsumerRegisterWorker(
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                consumer.Received += async (sender, eventArgs) =>
                {
                    ...
                    var message = JsonConvert.DeserializeObject<AddWorkerCommand>(contentString);
                    _ = await _mediator.Send(message, stoppingToken);
                };

                _channel.BasicConsume(MessageType.RegisterWorker.GetDescription(), true, consumer);
            }
            catch (Exception e)
            {
